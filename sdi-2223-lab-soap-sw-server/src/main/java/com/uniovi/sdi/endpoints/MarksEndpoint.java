...
public class MarksEndpoint {
    ...
    @Autowired
    public MarksEndpoint(MarksRepository markRepository) { this.marksRepository = markRepository;
}

@PayloadRoot(namespace = NAMESPACE_URI, localPart = "getMarksRequest") @ResponsePayload
public GetMarksResponse getMarks(@RequestPayload GetMarksRequest request) {
    GetMarksResponse response = new GetMarksResponse(); response.setUser(marksRepository.findAllByUser(request.getDni())); return response;
} }